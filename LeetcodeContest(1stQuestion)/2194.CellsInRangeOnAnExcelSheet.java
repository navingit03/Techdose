...
class Solution {
    public List<String> cellsInRange(String s) {
        ...
        String []arr=s.split(":");
        for(char ch=arr[0].charAt(0);ch<=arr[1].charAt(0);ch++ ) //col represented as character
        {
            for(char i=arr[0].charAt(1);i<=arr[1].charAt(1);i++) //row represented as integer
            {
                list.add(ch+""+i); //adding char+int col wise representation
            }
        }
        return list;
    }
