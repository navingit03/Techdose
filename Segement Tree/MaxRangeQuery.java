public class MaxRangeQuery {
    ...
    public static void buildST(int stind,int arr[],int s,int e) {
        ...
        st[stind]=Math.max(st[2*stind+1], st[2*stind+2]); //updating parent node with max of childs
    }
    public static int query(int stidx,int s,int e,int qs,int qe){
        if(qe<s||e<qs) return Integer.MIN_VALUE; //no overlap so min val
        ...
        int mid=s+(e-s)/2;
        //partial overlap
        int leftMax=query(2*stidx+1, s, mid, qs, qe);
        int rightMax=query(2*stidx+2, mid+1, e, qs, qe);
        return Math.max(leftMax, rightMax); //max of left and right query
    }
    public static void updatenode(int stidx,int s,int e,int pos,int val){
        ...
        st[stidx]=Math.max(st[2*stidx+1],st[2*stidx+2]); //updating parents
    }
    public static void main(String[] args) {
        int n=6;
        int arr[]={5,3,35,4,1,2};
